...
RosSimulation::RosSimulation() : Node("navigation_potential_function_simulation")
{
    ...
    vector<vector<double>> goal({{10.5, 10.5}});
    ...
    v_pid.set_PID(0.5, 0, 0);
    w_pid.set_PID(0.5, 0.1, 0);

    cmd_vel_publisher_ = this->create_publisher<geometry_msgs::msg::Twist>("/turtle" + to_string(robot_ID) +  "/cmd_vel", 10);
    pose_subscriber_ = this->create_subscription<turtlesim::msg::Pose>("/turtle1/pose", 10, std::bind(&RosSimulation::pose_callback, this, placeholders::_1));
...
void RosSimulation::robot_controller()
{
    ...
    double angular_velocity = w_pid.get_PID_control_input(error_dist, dt);
